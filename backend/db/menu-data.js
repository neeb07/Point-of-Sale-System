/**
 * Blaze Pizza House — the menu, transcribed from the printed card.
 *
 * Single source of truth. Both the fresh-install seed in database.js and the
 * re-seed script in scripts/seed_blaze_menu.js read from here, so the two can
 * never drift apart the way they had.
 *
 * Item shape:
 *   c  category
 *   n  name
 *   d  description (the ingredient list printed under the item)
 *   p  single fixed price
 *   v  [[label, price], ...] — sets has_variants; the base price is ignored
 *
 * Deal shape:
 *   g  deal group (the banner it sits under on the card)
 *   n  name
 *   p  price
 *   d  description, worded as the card words it
 *   items  [[menuItemName, quantity, variantLabel?], ...]
 */

const MENU = [
  // ══ BLAZE SPECIAL ═══════════════════ Medium 1450 / Large 2050 / XL 2950 ══
  ...
  // ══ STUFF CRUST ═════════════════════ Medium 1550 / Large 2150 / XL 3200 ══
  ...
  { c: 'Stuff Crust', n: 'Royal Crust Pizza',
    d: 'Mix Cheese, Chicken Tikka, Seekh Kabab, Special Pizza Sauce, Sweet Corn, Black Olives',
    v: [['Medium', 1550], ['Large', 2150], ['X-Large', 3200]] },
  { c: 'Stuff Crust', n: 'Crown Crust Pizza',
    d: 'Chicken Tikka, Mix Cheese, Black Olives, Tomato, Capsicum, Special Pizza Sauce',
    v: [['Medium', 1550], ['Large', 2150], ['X-Large', 3200]] },

  // ══ REGULAR PIZZA ══════════ Small 650 / Med 1250 / Large 1850 / XL 2750 ══
  ...
  // ══ CHINESE ═══════════════════════════════════════════════════════════════
  // Names are kept exactly as printed ("Chowmain", "Menchorian", "Black Paper")
  // so what staff see on the till matches the card in front of the customer.
  ...
  // ══ DRINK ═════════════════════════════════════════════════════════════════
  // The card prints two prices for each bottled size (e.g. "Rs. 80/100"),
  // which is the two brands carried. Entered as two variants per size.
 ...
 * Deals.
 *
 * The pizza deals carry the card's note that only a regular pizza is served,
 * so each links to a Regular Pizza flavour (Tikka) at the stated size.
 */
const DEALS = [
  // ══ PIZZA DEALS ═══════════════════════════════════════════════════════════
  { g: 'Pizza Deals', n: 'Pizza Deal 1', p: 850,
    d: 'Shawarma Roll + Small Pizza + Regular Drink (regular pizza only)',
    items: [['Shawarma Roll', 1], ['Tikka', 1, 'Small'], ['Regular Drink', 1, 'Option A']] },
  { g: 'Pizza Deals', n: 'Pizza Deal 2', p: 1599,
    d: 'Large Pizza + 1 Liter Drink (regular pizza only)',
    items: [['Tikka', 1, 'Large'], ['1 Liter Drink', 1, 'Option A']] },
  { g: 'Pizza Deals', n: 'Pizza Deal 3', p: 999,
    d: '2 Small Pizza + 2 Regular Drink (regular pizza only)',
    items: [['Tikka', 2, 'Small'], ['Regular Drink', 2, 'Option A']] },
  { g: 'Pizza Deals', n: 'Pizza Deal 4', p: 1999,
    d: '2 Medium Pizza + 500 ml Drink (regular pizza only)',
    items: [['Tikka', 2, 'Medium'], ['500ml Drink', 1, 'Option A']] },
  { g: 'Pizza Deals', n: 'Family Deal 5', p: 2999,
    d: '2 Large Pizza + 1.5 Liter Drink (regular pizza only)',
  ...
  { g: 'Zinger Deals', n: 'Zinger Deal 3', p: 1900,
    ...
    // Wings are only sold in 6s and 12s, so the 6-piece variant is the closest
    // stock unit for a "3 wings" deal line.
  ...
  { g: 'Platter Deals', n: 'Platter Deal 1', p: 3500,
    d: '2 Zinger Cheesy Wrap + 1 Chicken Tikka Medium + 1 Pizza Loaded Fries + 6 Hot Wings + 1.5 Drink',
  ...
  { g: 'Platter Deals', n: 'Platter Deal 3', p: 3500,
    d: '1 Medium Pizza (Regular) + 1 Creamy Baked Pasta + 1 Chowmein + 6 Crispy Wings + 6 Nuggets + 1.5 Drink',
