...
function buildPDF(data) {
  return new Promise((resolve, reject) => {
    ...
    // ── HEADER ──
    doc.rect(0, 0, doc.page.width, 75).fill(ORANGE);
    doc.fillColor(WHITE).font('Helvetica-Bold').fontSize(22)
       .text('TASTY BITES', 40, 18, { align: 'center' });
    ...
    const section = (title, romanTitle) => {
      doc.rect(40, y - 2, PAGE_W, 22).fill('#FFF7ED');
    ...
    row('Total Orders (Kul Orders):', totalOrd, true, ORANGE);
    ...
    if (topItems.length > 0) {
      ...
      // table header
      doc.rect(40, y, PAGE_W, 18).fill('#FFF7ED');
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.font('Helvetica').fontSize(8).fillColor(WHITE)
         .text(`Generated by Tasty Bites POS  |  Page ${i + 1} of ${pageCount}  |  ${new Date().toLocaleString('en-PK')}`,
